...
@section('content')
    ...
    <div class="mb-5 pb-5">
        <div class="featured-header d-flex justify-content-between align-items-center mb-5">
            ...
            <a href="{{ route('explore') }}" class="btn btn-outline">
        ...
        <div id="featuredCarousel" class="carousel slide" data-bs-ride="carousel">
            <div class="carousel-inner">
                @foreach($featuredCourses->chunk(4) as $chunkIndex => $chunk)
                    <div class="carousel-item {{ $chunkIndex == 0 ? 'active' : '' }}">
                        <div class="row g-4">
                            @foreach($chunk as $course)
                                @php $purchased = auth()->check() && auth()->user()->hasPurchased($course); @endphp
                                <div class="col-md-3">
                                    <a href="{{ $purchased ? route('learn.show', $course->slug) : route('courses.show', $course->slug) }}"
                                            ...
                                            <div class="course-content">
                                                <div class="course-instructor">
                                                    <div class="user-avatar" style="width: 24px; height: 24px; font-size: 10px;">
                                                        {{ strtoupper(substr($course->instructor->name ?? 'G', 0, 1)) }}</div>
                                                    {{ $course->instructor->name ?? 'Chuyên gia' }}
                                                </div>
                                                ...
                                                <div class="course-price">
                                                    ...
                                                    @else
                                                        @if($course->sale_price)
                                                            <span
                                                                class="current">{{ number_format($course->sale_price, 0, ',', '.') }}đ</span>
                                                            <span class="original">{{ number_format($course->price, 0, ',', '.') }}đ</span>
                                                        @else
                                                            <span class="current">{{ number_format($course->price, 0, ',', '.') }}đ</span>
                                                        @endif
                                                    @endif
                                                </div>
                                            </div>
